...
│   └── metaswitch_plugin   # The actual source code of the plugin, including device management, user interface properties, and rendering logic
│       ├── devices         # Folder containing files related to device enumeration and management
│       │   ├── enumerate.c  # Source file for enumerating available video capture devices on Linux/macOS
│       │   └── enumerate_win.c   # Source file for enumerating available video capture devices on Windows
│       ├── ui              # Folder containing files related to user interface properties and rendering logic
│       │   ├── countdown_ui.cpp     # C++ source code for implementing the countdown timer UI element
│       │   └── metaswitch_ui.cxx   # C++ source code for implementing the main plugin user interface component, including device selection and rotation schedule management
│       └── render          # Folder containing files related to rendering logic, such as video capture device handling and frame composition
│           ├── capture.cpp     # Source file for capturing frames from video capture devices on Linux/macOS
│           ├── capture_win.cpp   # Source file for capturing frames from video capture devices on Windows
│           └── renderer.cpp     # C++ source code for rendering the plugin's output, including frame composition and timing synchronization with other plugins or OBS Studio components
├── include                  # Folder containing header files required by the plugin, such as device enumeration headers and user interface properties definitions
│   └── metaswitch_plugin   # Header files for the main plugin source code, including device management, user interface properties, and rendering logic
├── resources                # Folder containing resource files used by the plugin during runtime, such as textures and audio clips
│   └── images               # Folder containing image files used in the plugin's UI elements and other graphics assets
├── plugins                  # Folder for storing compiled version of the plugin on Linux/macOS
│   └── metaswitch_plugin.so   # Shared object file generated by compiling the source code using CMake, which can be loaded into OBS Studio as a plugin
├── tests                   # Folder containing test cases and testing procedures for ensuring the stability and reliability of the plugin under various scenarios
│   └── metaswitch_plugin_tests.cpp   # Testing file that exercises different aspects of the plugin's functionality, such as device enumeration, user interface properties management, and rendering logic
├── build                  # Folder for storing temporary files generated during building the plugin on Linux/macOS
│   └── CMakeFiles        # Build directory used by CMake to generate object files from source code
└── doc                     # Folder containing documentation related to the plugin, such as tutorials and guides
```
