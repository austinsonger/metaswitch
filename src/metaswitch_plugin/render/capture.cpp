...
public:
    VideoCapture() : videoSource(nullptr), active(false) {
        videoSource = obs_source_create_private("video_capture", "Video Capture", nullptr);
    }
    ...
    void start() {
        if (!active) {
            active = true;
            captureThread = std::thread(&VideoCapture::captureLoop, this);
        }
    }
